...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    //Create a disjoint sets data structure in which each cell of the maze is represented as a separate item.
    DisjointSets set = new DisjointSets(horiz * vert);

    //Order the interior walls of the maze in a random order.

    //A temporary class to represent walls.
    class pair {
      private int x;
      private int y;
      private boolean bool; // Tell if horizontal wall.

      pair(int x, int y, boolean bool) {
        this.x = x;
        this.y = y;
        this.bool = bool;
      }
    ...
    pair[] walls = new pair[horiz * (vert - 1) + vert * (horiz - 1)];
    int index = 0;
    //Fill horizontal walls.
    for (int a = 0; a < vert - 1; a++) {
      for (int b = 0; b < horiz; b++) {
        walls[index] = new pair(b, a, true);
        index++;
      }
    }
    ...
    for (int a = 0; a < horiz - 1; a++) {
      for (int b = 0; b < vert; b++) {
        walls[index] = new pair(a, b, false);
        index++;
      }
    }
    //Order the walls randomly
    int w = walls.length;
    while(w > 1) {
      int r = randInt(w);
      pair temp = walls[w - 1];
    ...
    for(int a = 0; a < walls.length; a++) {
      int cell1;
      int cell2;
      cell1 = walls[a].y * horiz + walls[a].x;
      //System.out.println("x y " + walls[a].x + " " + walls[a].y);
      //System.out.println("cell1 " + cell1);
      if(walls[a].bool) {
        cell2 = (walls[a].y + 1) * horiz + walls[a].x;
      }
      else{
        cell2 = walls[a].y * horiz + walls[a].x + 1;
      }
      int root1 = set.find(cell1);
      int root2 = set.find(cell2);
      if(root1 != root2) {
        if(walls[a].bool)
          hWalls[walls[a].x][walls[a].y] = false;
        else
          vWalls[walls[a].x][walls[a].y] = false;
