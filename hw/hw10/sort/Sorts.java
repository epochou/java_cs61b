...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    // Replace the following line with your solution.
      int[] sortKeys = new int[keys.length];
      int[] count = new int[16];
      for(int i = 0; i < keys.length; i++) {
          int sortKey = keys[i];
          for(int j = 0; j < whichDigit; j++) { //Move the sort key to the last digit
              sortKey = sortKey >> 4;
          }
          sortKey = sortKey & 15;   //Extract the last digit
          sortKeys[i] = sortKey;
          count[sortKeys[i]]++;     //Compute frequency counts
      }
      int total = 0;
      for(int i = 0; i < 16; i++) { //Transform count to indices.
          int c = count[i];
          count[i] = total;
          total += c;
      }
